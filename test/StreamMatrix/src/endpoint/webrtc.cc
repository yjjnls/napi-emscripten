...
bool Webrtc::Initialize(Promise *promise)
{
    ...
    if (role_ != WebrtcRole::kOffer && role_ != WebrtcRole::kAnswer) {
        GST_ERROR("[Webrtc] %p initialize failed, invalid role: %s.", webrtc_, role_ ? "Answer" : "Offer");
        return false;
    }
    ...
    if (!app()->VideoEncoding().empty()) {
        GST_DEBUG("[Webrtc] %s (%p %s) media constructed: video", Id().c_str(), webrtc_, role_ ? "Answer" : "Offer");
    ...
    if (!app()->AudioEncoding().empty()) {
        GST_DEBUG("[Webrtc] %s (%p %s) media constructed: audio", Id().c_str(), webrtc_, role_ ? "Answer" : "Offer");
    ...
    if (ret == GST_STATE_CHANGE_FAILURE) {
        GST_DEBUG("[Webrtc] %s (%p %s) initialize failed.", Id().c_str(), webrtc_, role_ ? "Answer" : "Offer");
    }
    GST_DEBUG("[Webrtc] %s (%p %s) initialize done.", Id().c_str(), webrtc_, role_ ? "Answer" : "Offer");
                              ...
                              gpointer user_data G_GNUC_UNUSED)
{
    ...
    json meta;
    meta["topic"] = "webrtc@" + webrtc->Id();
    meta["origin"] = webrtc->app()->uname();
    meta["type"] = "ice";

    GST_LOG("[Webrtc] %p (%s) local candidate created.", webrtc->webrtc_, webrtc->role_ ? "Answer" : "Offer");
...
void Webrtc::on_sdp_created(GstPromise *promise, gpointer user_data)
{
    ...
    meta["origin"] = webrtc->app()->uname();
    meta["type"] = "sdp";
    meta["origin"] = webrtc->Id();

    g_signal_emit_by_name(webrtc->webrtc_, "set-local-description", sdp, nullptr);
    GST_INFO("[Webrtc] %s (%p %s) local description created.", webrtc->Id().c_str(), webrtc->webrtc_, webrtc->role_ ? "Answer" : "Offer");
...
void Webrtc::on_webrtc_pad_added(GstElement *webrtc_element, GstPad *new_pad, gpointer user_data)
{
    ...
    if (g_strcmp0(encoding_name, "VP8") == 0) {
        GST_INFO("[Webrtc] %s (%p %s) receive video: VP8.", webrtc->Id().c_str(), webrtc, webrtc->role_ ? "Answer" : "Offer");
        ...
        GST_INFO("[Webrtc] %s (%p %s) receive video: H264.", webrtc->Id().c_str(), webrtc, webrtc->role_ ? "Answer" : "Offer");
        out = gst_parse_bin_from_description(
        ...
        GST_INFO("[Webrtc] %s (%p %s) receive audio: PCMA.", webrtc->Id().c_str(), webrtc, webrtc->role_ ? "Answer" : "Offer");
        out = gst_parse_bin_from_description(
        ...
        GST_INFO("[Webrtc] %s (%p %s) receive video: OPUS.", webrtc->Id().c_str(), webrtc, webrtc->role_ ? "Answer" : "Offer");
        out = gst_parse_bin_from_description(
            // "rtppcmadepay ! tee name=local_audio_tee allow-not-linked=true ! queue ! alawdec ! audioconvert ! spectrascope shader=3 ! ximagesink sync=false",
...
void Webrtc::set_remote_description(Promise *promise)
{
    ...
    GST_INFO("[Webrtc] %s (%p %s) set remote description.", Id().c_str(), webrtc_, role_ ? "Answer" : "Offer");
...
void Webrtc::set_remote_candidate(Promise *promise)
{
    ...
    GST_LOG("[Webrtc] %p (%s) set remote candidate.", webrtc_, role_ ? "Answer" : "Offer");
}
...
void Webrtc::Terminate()
{
    ...
    GST_DEBUG("[Webrtc] %s %p (%s) terminate done.", Id().c_str(), webrtc_, role_ ? "Answer" : "Offer");
    if (webrtc_) {
